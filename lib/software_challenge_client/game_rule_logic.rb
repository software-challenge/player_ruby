...
class GameRuleLogic
  ...
  def self.is_bee_blocked(board, color)
    ...
    get_neighbours(board, bee_fields[0].coordinates).all? { |f| !f.empty? }
  end
  ...
  def self.validate_drag_move(gamestate, move)
    ...
    unless is_swarm_connected(board_without_piece)
  ...
  def self.validate_destination_next_to_start(move)
    unless is_neighbour(move.start, move.destination)
  ...
  def self.validate_bee_move(board, move)
    validate_destination_next_to_start(move)
    unless can_move_between(board, move.start, move.destination)
  ...
  def self.validate_grasshopper_move(board, move)
    unless two_fields_on_one_straight(move.start, move.destination)
    ...
    if get_line_between_coords(board, move.start, move.destination).any?(&:empty?)
  ...
  def self.get_line_between_coords(board, start, destination)
    unless two_fields_on_one_straight(start, destination)
      raise InvalidMoveException, 'destination is not in line with start'
    end
  ...
  def self.possible_set_move_destinations(board, owner)
    board.fields_of_color(owner)
         .flat_map { |f| get_neighbours(board, f).filter(&:empty?) }
         .uniq
    ...
    set_destinations =
      if undeployed.size == STARTING_PIECES.size
        ...
        if gamestate.undeployed_pieces(gamestate.other_player_color).size == STARTING_PIECES.size
          ...
          gamestate.board.field_list.filter(&:empty?)
        else
